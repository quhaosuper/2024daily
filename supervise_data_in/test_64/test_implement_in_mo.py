from test_0630.supervise_data_in.test_64.implement_module import *


class Test_insert:
    # @pytest.mark.parametrize("input_d",
    #                          yaml.safe_load(open("./0519预中标供应商报价贴近预算.yml", encoding='utf-8')))
    @pytest.mark.parametrize("input_d",
    ...
    def test_all_1(self, input_d):
        ...
        if dic_excute['invite_bid'] == 0:
            ...
            for i in range(len(input_d['dic_purchase_file_section'])):
                input_d['dic_purchase_file_section'][i]['PURCHASE_FILE_ID'] = list_purchase_file_id[i]
                input_d['dic_purchase_file_section'][i]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                purchase_file_section_in(input_d['dic_purchase_file_section'][i])
            for _ in range(len(input_d['dic_section_clause_type'])):
                list_eval_lv1_id.append([])
            for i in range(len(input_d['dic_section_clause_type'])):
                for j in range(len(input_d['dic_section_clause_type'][i])):
                    ...
                    list_eval_lv1_id[i].append(purchase_section_clause_type_in(input_d['dic_section_clause_type'][i][
                                                                                   j]))
            for i in range(len(input_d['dic_purchase_section_clause'])):
                for j in range(len(input_d['dic_purchase_section_clause'][i])):
                    for h in range(len(input_d['dic_purchase_section_clause'][i][j])):
                        input_d['dic_purchase_section_clause'][i][j][h]['EVAL_LEVEL1_ID'] = list_eval_lv1_id[i][j]
                        purchase_section_clause_in(input_d['dic_purchase_section_clause'][i][j][h])
            for i in range(len(input_d['dic_section_technical_requirement'])):
                for j in range(len(input_d['dic_section_technical_requirement'][i])):
                    ...
                    input_d['dic_section_technical_requirement'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[
                        i]
            ...
            for i in range(len(input_d['dic_section_special_qualification'])):
                for j in range(len(input_d['dic_section_special_qualification'][i])):
                    ...
                    input_d['dic_section_special_qualification'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[
                        i]
            ...
            for i in range(len(input_d['dic_tender_notice_section'])):
                for j in range(len(input_d['dic_tender_notice_section'][i])):
                    input_d['dic_tender_notice_section'][i][j]['TENDER_NOTICE_ID'] = list_tender_notice_id[j]
            ...
            for i in range(len(input_d['dic_file_download'])):
                for j in range(len(input_d['dic_file_download'][i])):
                    input_d['dic_file_download'][i][j]['PURCHASE_PROJECT_ID'] = purchaseprojectid
                    input_d['dic_file_download'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                    purchase_file_download_in(input_d['dic_file_download'][i][j])
        ...
        if dic_excute['open_bid'] == 0:
            ...
            for i in range(len(input_d['dic_supplier_list'])):
                for j in range(len(input_d['dic_supplier_list'][i])):
                    input_d['dic_supplier_list'][i][j]['PURCHASE_PROJECT_ID'] = purchaseprojectid
                    input_d['dic_supplier_list'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                    input_d['dic_supplier_list'][i][j]['OPEN_BID_ID'] = list_openbidid[i]
                    purchase_supplier_list_in(input_d['dic_supplier_list'][i][j])
            for i in range(len(input_d['dic_purchase_section'])):
        ...
        list_eval_com = []
        list_eval_com_id = []
        list_eval_com_sectid = []
        if dic_excute['evaluation_bid'] == 0:
            print(purchaseprojectid, purchaseprojectname)
            return
        else:
            for i in range(len(input_d['dict_evaluation_report'])):
                for j in range(len(input_d['dict_evaluation_report'][i])):
                    input_d['dict_evaluation_report'][i][j]['PURCHASE_PROJECT_ID'] = purchaseprojectid
                    input_d['dict_evaluation_report'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                    list_eval = evaluation_report_in(input_d['dict_evaluation_report'][i][j])
                    list_eval_rep_id.append(list_eval[0])
                    list_eval_sectionid.append(list_eval[1])
            for i in range(len(input_d['dict_eval_committee'])):
                for j in range(len(input_d['dict_eval_committee'][i])):
                    input_d['dict_eval_committee'][i][j]['PURCHASE_PROJECT_ID'] = purchaseprojectid
                    input_d['dict_eval_committee'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                    list_eval_com = eval_committee_in(input_d['dict_eval_committee'][i][j])
                    list_eval_com_id.append(list_eval_com[0])
                    list_eval_com_sectid.append(list_eval_com[1])
            for i in range(len(input_d['dict_expert_list'])):
                for j in range(len(input_d['dict_expert_list'][i])):
                    input_d['dict_expert_list'][i][j]['PURCHASE_PROJECT_ID'] = purchaseprojectid
                    input_d['dict_expert_list'][i][j]['PURCHASE_SECTION_ID'] = list_eval_com_sectid[i]
                    input_d['dict_expert_list'][i][j]['EVAL_COMMITTEE_ID'] = list_eval_com_id[i]
                    expert_list_in(input_d['dict_expert_list'][i][j])
            for i in range(len(input_d['dict_supplier_score_detail'])):
                for j in range(len(input_d['dict_supplier_score_detail'][i])):
                    input_d['dict_supplier_score_detail'][i][j]['PURCHASE_PROJECT_ID'] = purchaseprojectid
                    input_d['dict_supplier_score_detail'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                    supplier_score_detail_in(input_d['dict_supplier_score_detail'][i][j])
            for i in range(len(input_d['dict_score_detail'])):
                for j in range(len(input_d['dict_score_detail'][i])):
                    input_d['dict_score_detail'][i][j]['PURCHASE_PROJECT_ID'] = purchaseprojectid
                    input_d['dict_score_detail'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                    score_detail_in(input_d['dict_score_detail'][i][j])
            for i in range(len(input_d['dict_candidate_list'])):
                for j in range(len(input_d['dict_candidate_list'][i])):
                    input_d['dict_candidate_list'][i][j]['PURCHASE_PROJECT_ID'] = purchaseprojectid
                    input_d['dict_candidate_list'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                    candidate_list_in(input_d['dict_candidate_list'][i][j])
            ...
            for i in range(len(input_d['dict_neg_offer'])):
                for j in range(len(input_d['dict_neg_offer'][i])):
                    input_d['dict_neg_offer'][i][j]['PURCHASE_PROJECT_ID'] = purchaseprojectid
                    input_d['dict_neg_offer'][i][j]['PURCHASE_SECTION_ID'] = list_eval_sectionid[i]
                    input_d['dict_neg_offer'][i][j]['EVALUATION_EVAL_REPORT_ID'] = list_eval_rep_id[i]
                    neg_offer_in(input_d['dict_neg_offer'][i][j])
            ...
            for i in range(len(input_d['dict_core_review_result'])):
                for j in range(len(input_d['dict_core_review_result'][i])):
                    input_d['dict_core_review_result'][i][j]['CORE_PRODUCT_REVIEW_ID'] = list_core_rev_id[i]
                    input_d['dict_core_review_result'][i][j]['PURCHASE_SECTION_ID'] = list_core_sectionid[i]
                    core_product_review_result_in(input_d['dict_core_review_result'][i][j])
            for i in range(len(input_d['dict_core_review_score'])):
                for j in range(len(input_d['dict_core_review_score'][i])):
                    input_d['dict_core_review_score'][i][j]['CORE_PRODUCT_REVIEW_ID'] = list_core_rev_id[i]
                    input_d['dict_core_review_score'][i][j]['PURCHASE_SECTION_ID'] = list_core_sectionid[i]
                    core_product_review_score_in(input_d['dict_core_review_score'][i][j])
        ...
        list_excute_ui.append('专家打分细项')
        list_excute_ui.append('预中标供应商')
        ...
        if dic_excute['win_bid'] == 0:
            ...
            for i in range(len(input_d['dict_performance_evaluation_score'])):
                for j in range(len(input_d['dict_performance_evaluation_score'][i])):
                    ...
                    input_d['dict_performance_evaluation_score'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[
                        i]
            ...
            for i in range(len(input_d['dict_win_supplier'])):
                for j in range(len(input_d['dict_win_supplier'][i])):
                    input_d['dict_win_supplier'][i][j]['WIN_NOTICE_ID'] = winnoticeid
                    input_d['dict_win_supplier'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                    win_supplier_in(input_d['dict_win_supplier'][i][j])
            for i in range(len(input_d['dict_bid_object_detail'])):
                for j in range(len(input_d['dict_bid_object_detail'][i])):
                    input_d['dict_bid_object_detail'][i][j]['WIN_NOTICE_ID'] = winnoticeid
                    input_d['dict_bid_object_detail'][i][j]['PURCHASE_SECTION_ID'] = list_purchasesectionid[i]
                    input_d['dict_bid_object_detail'][i][j]['PURCHASE_SECTION_DETAIL_ID'] = \
                        list_purchasesectiondetailid[i]
                    bid_object_detail_in(input_d['dict_bid_object_detail'][i][j])

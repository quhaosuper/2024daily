import os
from test_0630.supervise_data_in.test_64.supervision_module import *
...
class Test_insert:
    ...
    def test_all_1(self, input_d):
        ...
        for i in range(len(input_d['dic_requirment_first_approval'])):
            input_d['dic_requirment_first_approval'][i]['ORIGIN_REQUIRE_ID'] = require_id
            ...
            list_re_pro_id.append(tup_first_approval[1])
        ...
        for i in range(len(input_d['dic_requirment_review_approval'])):
            input_d['dic_requirment_review_approval'][i]['ORIGIN_REQUIRE_ID'] = require_id
        ...
        # 创建一个数据列表
        data = [
            ['yearProjectId', 'quarProjectId', 'planApprovalId', 'constructionPlanId', 'requireId',
             'intentionNoticeId', 'intentionNoticeDetailId', 'intentionAdviceId', 'requireSurveyId',
             'reqresearchDetailId', 'demonstrationId', 'demonstrationDetailId', 'requireSectionId',
             'requireSectionDetailId', 'obejectTechParameterId', 'requireSectionSupplierQualId', 'ecoId',
             'requireNoticeId', 'requireCheckId', 'id'],
            [year_project_id, quar_project_id, random.choice(list_plan_approval_id), con_pl_id, require_id,
             random.choice(list_intention_notice_id), random.choice(list_intention_detail_id), random.choice(
                list_intention_advice_id), random.choice(list_require_survey_id), random.choice(list_surveydetail),
             random.choice(list_purchase_demonstration), random.choice(list_demonstration_detail), random.choice(
                list_require_section_id), random.choice(list_section_detail_id), random.choice(
                list_tech_parameter_id), random.choice(list_supplier_qual_id), random.choice(list_eco_id),
             random.choice(list_require_notice_id), random.choice(list_require_check_id), random.choice(
                list_submit_id_list)]
        ]
        # 按顺序写入数据
        for row in data:
            sheet.append(row)
        # 定义相对路径
        file_name = '计划需求主键表.xlsx'
        directory = '../../db_compare/'
        # 合并路径
        file_path = os.path.join(directory, file_name)
        workbook.save(file_path)
        print(f'Excel 文件已生成到相对路径：{file_path}')
        print(o_require_id)

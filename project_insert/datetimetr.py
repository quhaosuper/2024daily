...
def datenow_hms():
    datenow = datetime.datetime.now()
    today_hms = datenow.strftime("%Y%m%d%H%M%S") + str(random.randint(0, 9999))
    return today_hms
...
# print(datenow_tstamp_plus_d(-10))

def datenow_tstamp_plus_m(a):
    datenow = datetime.datetime.now() + relativedelta(minutes=a)
    today_hms = datenow.strftime("%Y-%m-%d %H:%M:%S")
    time_stamp = time.mktime(time.strptime(today_hms, '%Y-%m-%d %H:%M:%S'))
    time_stamp = round(time_stamp)*1000
    return time_stamp
...
def datenow_tstamp_plus_s(a):
    datenow = datetime.datetime.now() + relativedelta(seconds=a)
    today_hms = datenow.strftime("%Y-%m-%d %H:%M:%S")
    time_stamp = time.mktime(time.strptime(today_hms, '%Y-%m-%d %H:%M:%S'))
    time_stamp = round(time_stamp)*1000
    return time_stamp
